...
> **Note:** This repository is forked from [microsoft/playwright-mcp](https://github.com/microsoft/playwright-mcp) and developed based on version 0.0.32. The main update includes enhanced span navigation capabilities for improved page interaction.
...
```js
{
  "mcpServers": {
    "playwright": {
      "command": "npx",
      "args": [
        "@lockon0927/playwright-mcp-with-chunk@latest"
      ]
    }
  }
}
...
```bash
claude mcp add playwright npx @lockon0927/playwright-mcp-with-chunk@latest
```
</details>
...
Go to `Cursor Settings` -> `MCP` -> `Add new MCP Server`. Name to your liking, use `command` type with the command `npx @lockon0927/playwright-mcp-with-chunk`. You can also verify config or add command like arguments via clicking `Edit`.
...
Go to `Advanced settings` -> `Extensions` -> `Add custom extension`. Name to your liking, use type `STDIO`, and set the `command` to `npx @lockon0927/playwright-mcp-with-chunk`. Click "Add Extension".
</details>
...
```bash
# For VS Code
code --add-mcp '{"name":"playwright","command":"npx","args":["@lockon0927/playwright-mcp-with-chunk@latest"]}'
```
...
```bash
npx @lockon0927/playwright-mcp-with-chunk@latest --config path/to/config.json
```
...
```bash
npx @lockon0927/playwright-mcp-with-chunk@latest --port 8931
```
...
import { createConnection } from '@lockon0927/playwright-mcp-with-chunk';
